...
class ChatsModelTest(APITestCase):
    def setUp(self):
        self.a_song = Chats.objects.create(
...
class BaseViewTest(APITestCase):
    ...
    def create_song(title="", artist=""):
        ...
        """
        if title != "" and artist != "":
            Chats.objects.create(title=title, artist=artist)
...
class GetAllChatsTest(BaseViewTest):

    def test_get_all_Chats(self):
        """
        This test ensures that all Chats added in the setUp method
        exist when we make a GET request to the Chats/ endpoint
        """
        self.login_client('test_user', 'testing')
        # hit the API endpoint
        response = self.client.get(
            reverse("Chats-list-create", kwargs={"version": "v1"})
        )
        # fetch the data from db
        expected = Chats.objects.all()
        serialized = ChatsSerializer(expected, many=True)
        self.assertEqual(response.data, serialized.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)


class GetASingleChatsTest(BaseViewTest):
    ...
    def test_get_a_song(self):
        ...
        # fetch the data from db
        expected = Chats.objects.get(pk=self.valid_song_id)
        serialized = ChatsSerializer(expected)
        self.assertEqual(response.data, serialized.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class AddChatsTest(BaseViewTest):
...
class UpdateChatsTest(BaseViewTest):
...
class DeleteChatsTest(BaseViewTest):
